@extends('layouts.app')
@section('title', 'Manajemen Buku Panduan')
  ...
  <div class="space-y-5">
    ...
    <div class="flex items-start justify-between gap-4">
      <div>
        ...
        <p class="mt-2 pasih-page-breadcrumb">
          ...
          <span>Manajemen Buku Panduan</span>
        </p>
      </div>

      <a href="{{ route('admin.guides.create') }}" class="pasih-add-btn inline-flex items-center gap-2 h-11 px-4 rounded-xl bg-blue-950 text-white text-sm font-semibold hover:bg-blue-900">
    ...
    <div class="rounded-xl bg-white ring-1 ring-slate-200 overflow-hidden">
      <div class="px-4 py-3 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <form method="GET" action="{{ route('admin.guides.index') }}" class="flex items-center gap-2 text-sm text-slate-700">
          <span>Tampil</span>
        ...
        <form method="GET" action="{{ route('admin.guides.index') }}" class="flex items-center gap-2 text-sm text-slate-700">
          ...
          <input id="q" type="text" name="q" value="{{ $search }}" class="h-8 w-48 px-3 rounded-md border border-[#B9B9B9] text-sm">
        ...
        <table class="w-full text-sm">
          <thead class="bg-slate-50 text-slate-600">
            <tr>
              ...
              <th class="px-4 py-3 text-center">Buku Panduan</th>
          ...
          <tbody>
            @forelse($guides as $guide)
              @php
                $rowNumber = ($guides->firstItem() ?? 1) + $loop->index;
              @endphp
              <tr class="border-t border-slate-100 text-slate-700">
                <td class="px-4 py-3 text-center">{{ $rowNumber }}</td>
                <td class="px-4 py-3">{{ $guide->document_title ?: '-' }}</td>
                <td class="px-4 py-3">{{ $guide->uploader?->name ?? '-' }}</td>
                <td class="px-4 py-3">{{ optional($guide->created_at)->format('d-m-Y H:i') ?: '-' }}</td>
                <td class="px-4 py-3 text-center">
                  @if(!empty($guide->file_path))
                    <a href="{{ asset('storage/'.$guide->file_path) }}" target="_blank" class="inline-flex w-full items-center justify-center text-rose-600 hover:underline" title="Lihat Buku Panduan" aria-label="Lihat Buku Panduan">
                ...
                <td class="px-4 py-3">
                  <div class="flex items-center gap-1.5">
                    <a href="{{ route('admin.guides.show', $guide) }}" class="h-8 w-8 rounded-md bg-blue-600 text-white inline-flex items-center justify-center" title="Detail">
                    ...
                    <a href="{{ route('admin.guides.edit', $guide) }}" class="h-8 w-8 rounded-md bg-amber-400 text-white inline-flex items-center justify-center" title="Edit">
                    ...
                    <form method="POST" action="{{ route('admin.guides.destroy', $guide) }}" data-confirm-type="delete" data-confirm-message="Apakah Anda yakin ingin menghapus data ini?">
                      @csrf
                      @method('DELETE')
            ...
            @empty
              <tr>
                <td colspan="6" class="px-4 py-6 text-center text-slate-500">Belum ada buku panduan.</td>
              </tr>
            @endforelse
          </tbody>
        </table>
      ...
      <div class="px-4 py-3 border-t border-slate-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-sm text-slate-600">
        <div>
          Menampilkan {{ $guides->firstItem() ?? 0 }} - {{ $guides->lastItem() ?? 0 }} dari {{ $guides->total() }} data
        </div>
        <div>
          {{ $guides->onEachSide(1)->links('vendor.pagination.pasih') }}
        </div>
      </div>
    </div>
  </div>
