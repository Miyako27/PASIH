@extends('layouts.app')
@section('title', 'Edit Buku Panduan')
    ...
    <div class="rounded-xl bg-white ring-1 ring-slate-200 overflow-hidden">
      ...
      <form method="POST" action="{{ route('admin.guides.update', $guide) }}" enctype="multipart/form-data" class="p-4 space-y-4">
        ...
        <label class="block text-sm font-semibold text-slate-700">
          ...
          <input type="text" name="document_title" value="{{ old('document_title', $guide->document_title) }}" required placeholder="Masukkan Judul Dokumen" class="mt-2 w-full h-10 px-4 py-2 rounded-md border border-[#B9B9B9] text-sm placeholder:text-[14px]">
        ...
        <div>
          <label class="block text-sm font-semibold text-slate-700">
            Upload Ulang Buku Panduan <span class="text-red-500">*</span>
          </label>
